...
class SQLDataset(Base):
    ...
    def tokenize_query(cls, query):
        ...
        curr = []
        delims = '()!=><+'
        aug = query.rstrip(';').replace('"', "'").replace('2,1', '1').replace('whereclaim', 'where claim')
        for c in delims:
        ...
        in_str = False
        for t in toks:
            if t.startswith("'") and not in_str:
                in_str = True
                continue
            if in_str and t == "'":
                in_str = False
...
if __name__ == '__main__':
    ...
    torch.save(proc, 'cache/data_nl2sql_sparc_cosql.debug.pt' if args.debug else 'cache/data_nl2sql_sparc_cosql.pt')
